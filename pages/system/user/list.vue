<template>
	<view class="fix-top-window">
		...
		<view class="uni-container">
			<unicloud-db ref="udb" collection="uni-id-users,asst-companies,uni-id-roles"
				field="company_id{name},username,mobile,status,email,role{role_name},dcloud_appid,tags,last_login_date" :where="where"
				...
				<uni-table ref="table" :loading="loading" :emptyText="error.message || $t('common.empty')" border stripe
					...
					<uni-tr>
						<uni-th align="center" sortable @sort-change="sortChange($event, 'company_id')">关联企业</uni-th>
					...
					<uni-tr v-for="(item,index) in data" :key="index">
						<uni-td align="center">{{item.company_id}}</uni-td>
...
<script>
	...
	const dbOrderBy = 'company_id asc'
	const dbSearchFields = ['company_id.name', 'username', 'role.role_name', 'mobile', 'email'] // 支持模糊搜索的字段列表
	// 分页配置
	const pageSize = 20
	const pageCurrent = 1
		...
		methods: {
			onqueryload(data) {
				for (var i = 0; i < data.length; i++) {
					let item = data[i]
					const companyArr = item.company_id.map(item => item.name)
					item.company_id = companyArr[0]
