      ...
      <unicloud-db ref="udb" :collection="collectionList" :where="where" page-data="replace"
        ...
        <uni-table ref="table" :loading="loading" :emptyText="error.message || '没有更多数据'" border stripe type="selection" @selection-change="selectionChange">
          <uni-tr>
            ...
            <uni-th align="center">用户标签tagid</uni-th>
          ...
          <uni-tr v-for="(item,index) in data" :key="index">
            ...
            <uni-td align="center">{{item.tags && item.tags[0] && item.tags[0].text}}</uni-td>
            <uni-td align="center">
              <view class="uni-group">
...
<script>
  ...
  export default {
    data() {
      return {
        collectionList: [ db.collection('uni-id-users').field('company_id,username,status,mobile,email,last_login_date,role,tags').getTemp(),db.collection('asst-companies').field('_id, name as text').getTemp(),db.collection('uni-id-roles').field('role_id, role_name as text').getTemp(),db.collection('opendb-app-list').field('appid, name as text').getTemp(),db.collection('uni-id-tag').field('tagid, name as text').getTemp() ],
        query: '',
        where: '',
        orderby: dbOrderBy,
        orderByFieldName: "",
