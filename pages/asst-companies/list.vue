...
<script>
  ...
  export default {
    data() {
      return {
        collectionList: [ db.collection('asst-companies').field('name,city_id,create_date').getTemp(),db.collection('opendb-city-china').field('code, name as text, eq(type, 2) as isleaf').getTemp() ],
        query: '',
        where: '',
        orderby: dbOrderBy,
        orderByFieldName: "",
